...
for d in "${tests[@]}"; do
  ...
  # if [ "$status" = "OK" ]; then
  #   if [ -x ./checker ]; then
  #     ./checker "$input" "$userout" "$expected" > checker_out.txt 2> checker_err.txt
  #     chk_rc=$?
  #     if [ $chk_rc -ne 0 ]; then
  #       status="WA"
  #       # In ra thông báo chi tiết từ checker
  #       if [ -s checker_err.txt ]; then
  #         echo "WA reason: $(cat checker_err.txt)"
  #       elif [ -s checker_out.txt ]; then
  #         echo "WA reason: $(cat checker_out.txt)"
  #       else
  #         echo "WA reason: checker failed"
  #       fi
  #     fi
  #   else
  #     if ! diff -q <(tr -s ' \t\r\n' '\n' < "$userout") <(tr -s ' \t\r\n' '\n' < "$expected") >/dev/null; then
  ...
  # debug
  # rm -f "$timefile" "$stderrfile" checker_err.txt checker_out.txt
